...
<div class="form-group mb-3">
    ...
    <input
        class="form-control list-tagify"
        name="type"
        data-list="{{ json_encode($types) }}"
        value="{{ Arr::get($attributes, 'type') }}"
        placeholder="{{ __('Select Type from the list') }}"
    >
</div>

<div class="form-group mb-3">
    <label class="control-label">{{ __('Bedrooms') }}</label>
    {!! Form::customSelect(
        'bedrooms',
        $bedrooms,
        Arr::get($attributes, 'bedrooms', 0),
    ) !!}
</div>
<div class="form-group mb-3">
    ...
    {!! Form::customSelect(
        'categories',
        $categories,
        Arr::get($attributes, 'categories', 0),
    ) !!}
</div>
